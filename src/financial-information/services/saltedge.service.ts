...
import {
  leadPayloadGenerator,
  createConnectionPayload,
} from '../mockRequest/connection';
...
export class SaltEdge {
  ...
  async getConsentWithConnectionId(connectionId: string) {
    const response = await httpClient.get(
      `partner_consents?connection_id=${connectionId}`,
    );
    return response.data.data;
  }
   ...
   * @returns list of active connections for fetching user accounts
   */
  async fetchTransaction(saltEdgeCustomerId: string) {
    const connections = await this.fetchConnection(saltEdgeCustomerId);
    const response = await Promise.all(
      connections.map(async (connection) => {
        const response = await httpClient.get(
          `transactions?connection_id=${connection.id}`,
        );
        return response.data.data;
      }),
    );

    const result = response.filter((item) => item != 0).flat();
    return result;
  }
}
