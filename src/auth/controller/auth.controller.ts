...
import {
  ...
  UpdatePasswordDTO,
  Verify2FaToken,
} from '../dtos/resendRegistration.dto';
...
export class AuthController {
  ...
  async resendRegistration(
    @Body() resendRegistrationDto: ResendRegistationDTO,
  ): Promise<IResponseMessage> {
    const response = await this.authServices.resendRegistrationToken(
      resendRegistrationDto.email,
    );
  ...
  @ApiResponse({ status: 500, description: 'Internal Server Error' })
  @Get('verify/:token')
  async verifyRegistrationEmailLink(
  ...
  async googleAuthRedirect(
    @Body() googleSignToken: GoogleSignDto,
  ): Promise<IResponseMessage> {
  ...
  async createBusinessOwnerInformation(
    @Request() req,
  ): Promise<IResponseMessage> {
  ...
  async resetPassword(
    @Body() resetPasswordDto: ResendRegistationDTO,
  ): Promise<IResponseMessage> {
  ...
  @Post('verify-2fa')
  async verify2fa(
    @Body() verify2FaTokenPayload: Verify2FaToken,
  ): Promise<IResponseMessage> {
    const response = await this.authServices.verify2fa(
      verify2FaTokenPayload.token,
    );
    return wrapResponseMessage(
      'verification email send successfully',
      response,
