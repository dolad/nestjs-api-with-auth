...
export const UnauthoriedBusinessEntityEndpoint = () => {
    ...
    it('/business-entity/kyc/stage-1 (POST) should not create if not logged in', async() => {
        ...
        const response = await request.post('/business-entity/kyc/stage-1').send(mockkycStage1);
...
export const businessEntityEndpoint = () => {
    ...
    it('/business-entity/kyc/stage-1 (POST) should not with wrong owner details', async() => {
        const response = await request.post('/business-entity/kyc/stage-1').send(invalidMockkycStage1).set('Authorization', `Bearer ${token}`);;
        expect(response.body.status).toBe(false);
    ...
    it('/business-entity/kyc/stage-1 (POST) should create if not logged in', async() => {
        const response = await request.post('/business-entity/kyc/stage-1').send(mockkycStage1).set('Authorization', `Bearer ${token}`);;
    ...
    it('/business-entity/kyc (GET) should create if not logged in', async() => {
        const response = await request.get('/business-entity/kyc').set('Authorization', `Bearer ${token}`);;
        expect(response.body.status).toBe(true);
